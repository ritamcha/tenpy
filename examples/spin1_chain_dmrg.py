...
Run from the root of a TenPy checkout, for example:
    ...
    python examples/spin1_chain_dmrg.py --n-sites 12 --j -16.1 --dz 0.379 --dpp -0.017 --case both
...
class _Spin1ChainDMRG:
    ...
    @property
    def sia_dz(self) -> float:
        raise NotImplementedError

    @property
    def sia_dpp(self) -> float:
        raise NotImplementedError
...
class Spin1ChainWithSIA(_Spin1ChainDMRG):
    """TenPy DMRG model with Dz (Sz)^2 and Dpp [(Sp)^2 + (Sm)^2] anisotropy."""
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "dz", _validate_finite_real(self.dz, "dz"))
        object.__setattr__(self, "dpp", _validate_finite_real(self.dpp, "dpp"))
...
def _build_case_model(
    ...
    if case == "with":
        return (
            Spin1ChainWithSIA(n_sites=n_sites, j_iso=j_iso, dz=dz, dpp=dpp, bc=bc, dmrg_settings=settings),
            "with SIA",
        )
    if case == "without":
        return (
...
def _run_case_worker(
    ...
    model, label = _build_case_model(case, n_sites, j_iso, dz, dpp, bc, settings)
    result = model.run_dmrg()
    return {
        "case": case,
        "label": label,
    ...
    *,
    ...
    if parallel and len(cases) > 1:
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(_run_case_worker, selected_case, n_sites, j_iso, dz, dpp, bc, settings)
    ...
    results = []
    for selected_case in cases:
        model, label = _build_case_model(selected_case, n_sites, j_iso, dz, dpp, bc, settings)
