...
def main() -> None:
    ...
    if args.case in ("with", "both"):
        model = Spin1ChainWithSIA(
            n_sites=args.n_sites,
            j_iso=args.j,
            d=args.d,
            dmrg_settings=settings,
        )
        run_case(model, "with SIA")

    if args.case in ("without", "both"):
        model = Spin1ChainWithoutSIA(
            n_sites=args.n_sites,
            j_iso=args.j,
            dmrg_settings=settings,
        )
        run_case(model, "without SIA")
